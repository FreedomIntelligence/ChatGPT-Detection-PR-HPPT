...
class ModelTrainer:
    def __init__(
        ...
        self.model = LogisticModel(RobertaForSequenceClassification.from_pretrained(model_name)).cuda()
    ...
    def load_data(self, filepath):
        ...
        for example in data:
            text = example["text"]
            label = example["jaccard_distance"]
    ...
    def evaluate_model(self, test_loader):
        ...
        with torch.no_grad():
            ...
            for inputs, labels in test_loader:
                outputs = self.model(inputs)
                output.extend(list(outputs.detach().cpu().numpy()))
    ...
    def run_training(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if best_loss > val_loss:
                best_loss = val_loss
                torch.save(self.model.state_dict(), "best_model_reg_MSE_Ja.pt")
            print(
                f"Epoch {epoch + 1}, train loss : {train_loss:.4f}, validation loss: {val_loss:.4f}")
        torch.save(self.model.state_dict(), "last_model_reg_MSE_Ja.pt")  
        # Load the best model based on the validation accuracy and evaluate it on the test dataset
        self.model.load_state_dict(torch.load("last_model_reg_MSE_Ja.pt"))
        test_loss = self.evaluate_model(test_loader)
        # Print the best accuracy and the classification report for the test dataset
        print(f"Best error: {test_loss:.4f}") 

if __name__ == "__main__":
    trainer = ModelTrainer("../Dataset/HPPT/train.json", 
                           "../Dataset/HPPT/val.json", 
                           "../Dataset/HPPT/test.json",
        model_path = "last_model_reg_MSE_Ja.pt"
    )
